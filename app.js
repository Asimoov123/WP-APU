...
getAccessToken(
  "9av7l7i045ybzxo5dqojmgigsyd3zc",
  "3sc3zyyboe8er68uy51ljq4409tpxn"
);
          ...
          db.query("SELECT COUNT(*) AS count FROM users;", (err, res2) => {
            ...
            db.query(
              ...
              (err, res3) => {
                ...
                fav_count = res3;
                db.query(
                  "SELECT id, name, surname, email FROM users LIMIT 10",
                  (err, res4) => {
                    if (err) throw err;
                    user_list = res4;
                    res.render("profile", {
...
app.post("/addfavorite", (req, res) => {
  const { name, cover, story, lifetime, developper, sales, genre, year } =
    req.body;
  let query =
    "INSERT INTO favorites (user_id, name, cover_url, story, lifetime, developer, sales, genre, release_year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
  db.query(
    query,
    [user_id, name, cover, story, lifetime, developper, sales, genre, year],
    (err, results) => {
      if (err) throw err;
...
app.post("/login", (req, res) => {
  ...
  db.query(query, [mail], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      return res
        .status(404)
        .json({ success: false, mail: false, password: false });
    }
  });
});
...
app.post("/updateUser", (req, res) => {
  ...
  if (nameRegex.test(surname) && nameRegex.test(name) && pwd1 === pwd2) {
    ...
    return res
      .status(200)
      .json({ success: true, name: true, surname: true, pwd: true });
  } else {
    let nameStatus = true;
    let surnameStatus = true;
    let pwdStatus = true;
...
function convertToMinutes(time) {
  ...
  for (const part of parts) {
    if (part.endsWith("h")) {
      // Extract the number before 'h' and convert to integer
      hours = parseInt(part.replace("h", ""));
    } else if (part.endsWith("m")) {
      // Extract the number before 'm' and convert to integer
